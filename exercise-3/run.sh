...
function ml() {
    ...
    ${MATLAB} ${MATLABFLAGS} -r "${ml_function};exit" ${ml_function}.m | sed -e "1,/${marker_line}/d" | python3 -c "import sys; print(sys.stdin.read().replace('\x1b\x5b\x3f\x31\x6c\x1b\x3e', '').strip())" > ${outfile}
}
